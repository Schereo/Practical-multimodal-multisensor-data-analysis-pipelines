...
def _pearson_correlation(df1: DataFrame, df2: DataFrame, df3: DataFrame):
    ...
    df2 = df2[['R1']]
    df3 = df3[['R1']]
    df1 = df1.rename(columns={'R1': f'R1_{df1_name}'})
    df2 = df2.rename(columns={'R1': f'R1_{df2_name}'})
    df3 = df3.rename(columns={'R1': f'R1_{df3_name}'})
...
def _rolling_pearson(df1: DataFrame, df2: DataFrame, df3: DataFrame, window_size=30):
    ...
    df2_name = df2.index.name
    df3_name = df3.index.name
    visualize_rolling_correlation(rolling_r_df1_df2, rolling_r_df1_df3, rolling_r_df2_df3, window_size, df1_name, df2_name, df3_name)
