...
def visualize_data(df1, df2, df3, show=False):
    ...
    _yearly__summed_percipitation(df1, df2, df3)
...
def _visualize_combined_data(df1, df2, df3, show=False):
    ...
    plt.ylabel('Precipitation in mm')
    plt.title(
        f'Precipitation in mm in {df1.index.name}, {df2.index.name}, {df3.index.name}')
...
def _yearly__summed_percipitation(df1, df2, df3):
    fig, axs = plt.subplots(2, 2, figsize=(7, 7))
    ...
    histo1 = sns.histplot(yearly_sum1, color='blue',
                          stat='percent', ax=axs[0, 0], legend=True, label='Test')
    histo1.legend([df1.index.name])
    ...
    fig.suptitle('Daily precipitation in mm')
    fig.tight_layout()
    plt.savefig(f'visualizations/combined/yearly_percipitation.png')
    plt.clf()
